	...
	for root,dirs,files in os.walk(data_dir):#遍历文件夹
		for file_node in files:
			...
			if(str(file_Name).endswith("xls") or str(file_Name).endswith("xlsx"))\
				...
				VOLTE_WLW_IMSI_NEW(file_Name)
			if(str(file_Name).endswith("xls") or str(file_Name).endswith("xlsx"))\
				and (str(file_node).find("补充物联网启用LTE用户号码批复表")!=-1):#物联网启用LTE用户号码批复表——IMSI/
				VOLTE_WLW_IMSI_OLD(file_Name)
			if(str(file_Name).endswith("xls") or str(file_Name).endswith("xlsx"))\
				and (str(file_node).find("中国移动网内启用物联网号段批复表")!=-1):#物联网启用LTE用户号码批复表——IMSI/
				VOLTE_WLW_MSISDN_NEW(file_Name)
			if(str(file_Name).endswith("xls") or str(file_Name).endswith("xlsx"))\
				and (str(file_node).find("补充中国移动网内启用物联网号段批复表")!=-1):#物联网启用LTE用户号码批复表——IMSI/
				VOLTE_WLW_MSISDN_OLD(file_Name)
def VOLTE_WLW_IMSI_NEW(file_Name):
...
def VOLTE_WLW_MSISDN_NEW(file_Name):
	route_bell,route_hw=ROUTE_NEW()
	workbook = xlrd.open_workbook(file_Name)
	table=workbook.sheets()[0]
	HW=""
...
def VOLTE_WLW_MSISDN_OLD(file_Name):
	route_bell,route_hw=ROUTE_NEW()
	workbook = xlrd.open_workbook(file_Name)
	table=workbook.sheets()[0]
	HW=""
